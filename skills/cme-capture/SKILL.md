...
0. If more than one agent writes the shared brain, open an isolation session (`cme-session`) and export `SECOND_BRAIN_ROOT`.
   Claim identity `grok-bot/content-media` (or `deep-agents/content-media` on Deep Agents).
