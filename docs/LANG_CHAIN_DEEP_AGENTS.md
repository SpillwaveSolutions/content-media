...
agent = create_deep_agent(
    ...
    skills=["./path/to/content-media/skills/"],
)
```
...
SkillsMiddleware(
    backend=backend,
    sources=["/skills/", "/path/to/content-media/skills/"],
)
```

```bash
npx skills add SpillwaveSolutions/content-media --skill '*' --yes
```
...
python3 path/to/second-brain-core/scripts/brain_session.py open \
  --repo "$BRAIN_REPO" \
  --bundle knowledge \
  --actor deep-agents/content-media \
...
```bash
export SECOND_BRAIN_IDENTITY="deep-agents/content-media"
