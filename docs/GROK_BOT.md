...
- Claim per process with `--author grok-bot/content-media` or `SECOND_BRAIN_IDENTITY=grok-bot/content-media`
- Do **not** use a single shared `knowledge/.identity.json` for a fleet.
- Chat prefix (optional): `Grok Bot: Media Consultant`
...
**Required:** type ownership. This pack may write: Article, Draft, Series, Subscriber, Segment, Performance. Refuse everything else.
