...
SCRIPT = ROOT / "scripts" / "cme_common.py"
...
def test_write_requires_author():
    with tempfile.TemporaryDirectory() as td:
        ...
        run("init-bundle", "--bundle", str(bundle), "--title", "Test", "--catalogs", "articles")
        r = run(
            "write",
            "--bundle",
            str(bundle),
...
def test_init_and_write():
    with tempfile.TemporaryDirectory() as td:
        bundle = Path(td) / "knowledge"
        r = run("init-bundle", "--bundle", str(bundle), "--title", "Test", "--catalogs", "articles")
        assert r.returncode == 0, r.stdout + r.stderr
        r = run(
            "write",
            "--bundle",
            ...
            "Hello World",
            "--author",
            "grok-bot/content-media",
        )
        assert r.returncode == 0, r.stdout + r.stderr
        data = json.loads(r.stdout)
        assert data["author"] == "grok-bot/content-media"
        assert data["event"]
...
def test_isolation_two_sessions_do_not_clobber():
    with tempfile.TemporaryDirectory() as td:
        ...
        r = run("init-bundle", "--bundle", str(knowledge), "--title", "Shared", "--catalogs", "articles")
        assert r.returncode == 0, r.stdout + r.stderr
        git(repo, "add", ".")
        ...
        assert (Path(sa["bundle"]) / "articles" / "session-a-note.md").exists()
        assert not (Path(sa["bundle"]) / "articles" / "session-b-note.md").exists()
        assert (Path(sb["bundle"]) / "articles" / "session-b-note.md").exists()
        assert not (Path(sb["bundle"]) / "articles" / "session-a-note.md").exists()
        ...
        assert (knowledge / "articles" / "session-a-note.md").exists()
        assert (knowledge / "articles" / "session-b-note.md").exists()


def test_sample_pack_walks():
    root = "Northstar Article"
